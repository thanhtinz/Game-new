...
public class BelieverTypeService : IBelieverTypeService
{
    ...
    public async Task ShiftBelieverTypesAsync(string religionId, string eventType, float magnitude)
    {
        ...
        int shift = Math.Max(1, (int)(religion.FollowerCount * magnitude * 0.05f));

        switch (eventType)
        {
            case "MiracleSuccess":
                // Casual → Devout
                int toDevout = Math.Min(shift, cas);
                cas -= toDevout; dev += toDevout;
                break;

            case "FailedMiracle":
                // Devout → Casual (doubt) + some → Heretic (crisis)
                int toCasual  = Math.Min(shift, dev);
                int toHeretic = Math.Min(shift / 3, dev - toCasual);
            ...
            case "HolyWar":
                ...
                int toFanatic = Math.Min(shift, dev);
                dev -= toFanatic; fan += toFanatic;
                break;

            case "HeresyTrial":
                // Heretic → either executed (removed) or driven underground (Cultist)
                int hereticsRemoved = Math.Min(shift / 2, her);
                int toCultist = Math.Min(shift / 2, her - hereticsRemoved);
                her -= (hereticsRemoved + toCultist);
                cul += toCultist;
                religion.FollowerCount = Math.Max(0, religion.FollowerCount - hereticsRemoved);
                break;

            case "Persecution":
                // Public believers go underground → Devout/Fanatic → Cultist
                int toCult = Math.Min(shift, dev + fan);
                int fromDev = Math.Min(toCult, dev);
            ...
            case "ProphecyFulfilled":
                ...
                int toDevout2 = Math.Min(shift * 2, cas);
                cas -= toDevout2; dev += toDevout2;
                break;
        }

        // Ensure no negatives
        religion.CasualCount   = Math.Max(0, cas);
        religion.DevoutCount   = Math.Max(0, dev);
        religion.FanaticCount  = Math.Max(0, fan);
        religion.CultistCount  = Math.Max(0, cul);
        religion.HereticCount  = Math.Max(0, her);
